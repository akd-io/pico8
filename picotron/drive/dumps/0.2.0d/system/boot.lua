...
-- load head manually -- might be used by startup.lua
-- same pattern as start of program generated by create_process();
-- startup does not have access to userland api otherwise (doesn't need to)
...
function run_userland_processes(allotment)
	...
	-- 0.1.1e: also happens because of atomic disk op locking -- reduced max safety from 4096 to 512 for speed

	while (keep_going and remaining > 0 and safety < 512) do
		keep_going = false
		local cpu = remaining / num -- for now, same for all processes. later: priority + be generous to process w/ locking disk op 
