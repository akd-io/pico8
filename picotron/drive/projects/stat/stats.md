...
```
333 results - 98 files

picotron/drive/dumps/ram/mount/bdbrewxk/main.lua:
  11  function _draw()
...
picotron/drive/dumps/ram/mount/biyphbik/undo.lua:
...
picotron/drive/dumps/ram/mount/dhboehmj/main.lua:
  35
  36: 	--print(stat(1),400,2,7)
  37  	--circ(mx,my,rr/2,7)

picotron/drive/dumps/ram/mount/dopyvbqs/main.lua:
  11  	end
  12: 	print(string.format("%.5f",stat(1)),2,2,7)
  13  end

picotron/drive/dumps/ram/mount/eapgbwvf/picotron_player.js:
  1: ⟪ 1589126 characters skipped ⟫){return function(p){return PATH.join2(root,p)}}var check=FS.readdir(mount.mountpoint).filter(isRealDir).map(toAbsolute(mount.mountpoint));while(check.length){var path=check.pop();var stat;try{stat=FS.stat(path)}catch(e){return callback(e)}if(FS.isDir(stat.mode)){check.push.apply(check,FS.readdir(path).filter(isRealDir).map(toAbsolute(path)))}entries[path]={timestamp:stat.mtime}}return callback(null,{type:"local",entries:entries})},getRemoteSet:function(mount,callback){var entries={};IDBFS.getDB(mount.mountpoint,function(err,db){if(err)return callback(err);try{var transaction=db.transaction([IDBFS.DB_STORE_NAME],"readonly");transaction.onerror=function(e){callback(this.error);e.preventDefault()};var store=transaction.objectStore(IDBFS.DB_STORE_NAME);var index=store.index("timestamp");index.openKeyCursor().onsuccess=function(event){var cursor=event.target.result;if(!cursor){return callback(null,{type:"remote",db:db,entries:entries})}entries[cursor.primaryKey]={timestamp:cursor.key};cursor.continue()}}catch(e){return callback(e)}})},loadLocalEntry:function(path,callback){var stat,node;try{var lookup=FS.lookupPath(path);node=lookup.node;stat=FS.stat(path)}catch(e){return callback(e)}if(FS.isDir(stat.mode)){return callback(null,{timestamp:stat.mtime,mode:stat.mode})}else if(FS.isFile(stat.mode)){node.contents=MEMFS.getFileDataAsTypedArray(node);return callback(null,{timestamp:stat.mtime,mode:stat.mode,contents:node.contents})}else{return callback(new Error("node type not supported"))}},storeLocalEntry:function(path,entry,callback){try{if(FS.isDir(entry.mode)){FS.mkdir(path,entry.mode)}else if(FS.isFile(entry.mode)){FS.writeFile(path,entry.contents,{canOwn:true})}else{return callback(new Error("node type not supported"))}FS.chmod(path,entry.mode);FS.utime(path,entry.timestamp,entry.timestamp)}catch(e){return callback(e)}callback(null)},removeLocalEntry:function(path,callback){try{var lookup=FS.lookupPath(path);var stat=FS.stat(path);if(FS.isDir(stat.mode)){FS.rmdir(path)}else if(FS.isFile(stat.mode)){FS.unlink(path)}}catch(e){return callback(e)}callback(null)},loadRemoteEntry:function(store,path,callback){var req=store.get(path);req.onsuccess=function(event){callback(null,event.target.result)};req.onerror=function(e){callback(this.error);e.preventDefault()}},storeRemoteEntry:function(store,path,entry,callback){var req=store.put(entry,path);req.onsuccess=function(){callback(null)};req.onerror=function(e){callback(this.error);e.preventDefault()}},removeRemoteEntry:function(store,path,callback){var req=store.delete(path);req.onsuccess=function(){callback(null)};req.onerror=function(e){callback(this.error);e.preventDefault()}},reconcile:function(src,dst,callback){var total=0;var create=[];Object.keys(src.entries).forEach(function(key){var e=src.entries[key];var e2=dst.entries[key];if(!e2||e.timestamp>e2.timestamp){create.push(key);total++}});var remove=[];Object.keys(dst.entries).forEach(function(key){var ⟪ 27970 characters skipped ⟫{follow:!dontFollow});var node=lookup.node;if(!node){throw new FS.ErrnoError(2)}if(!node.node_ops.getattr){throw new FS.ErrnoError(1)}return node.node_ops.getattr(node)},lstat:function(path){return FS.stat(path,true)},chmod:function(path,mode,dontFollow){var node;if(typeof path==="string"){var lookup=FS.lookupPath(path,{follow:!dontFollow});node=lookup.node}else{node=path}if(!node.node_ops.setattr){throw new FS.ErrnoError(1)}node.node_ops.setattr(node,{mode:mode&4095|node.mode&~4095,timestamp:Date.now()})},lchmod:function(path,mode){FS.chmod(path,mode,true)},fchmod:function(fd,mode){var stream=FS.getStream(fd);if(!stream){throw new FS.ErrnoError(9)}FS.chmod(stream.node,mode)},chown:function(path,uid,gid,dontFollow){var node;if(typeof path==="string"){var lookup=FS.lookupPath(path,{follow:!dontFollow});node=lookup.node}else{node=path}if(!node.node_ops.setattr){throw new FS.ErrnoError(1)}node.node_ops.setattr(node,{timestamp:Date.now()})},lchown:function(path,uid,gid){FS.chown(path,uid,gid,true)},fchown:function(fd,uid,gid){var stream=FS.getStream(fd);if(!stream){throw new FS.ErrnoError(9)}FS.chown(stream.node,uid,gid)},truncate:function(path,len){if(len<0){throw new FS.ErrnoError(22⟪ 5531 characters skipped ⟫coding=opts.encoding||"binary";if(opts.encoding!=="utf8"&&opts.encoding!=="binary"){throw new Error('Invalid encoding type "'+opts.encoding+'"')}var ret;var stream=FS.open(path,opts.flags);var stat=FS.stat(path);var length=stat.size;var buf=new Uint8Array(length);FS.read(stream,buf,0,length,0);if(opts.encoding==="utf8"){ret=UTF8ArrayToString(buf,0)}else if(opts.encoding==="binary"){ret=buf}FS.close(stream);return ret},writeFile:function(path,data,opts){opts=opts||{};opts.flags=opts.flags||"w";var stream=FS.open(path,opts.flags,opts.mode);if(typeof data==="string"){var buf=new Uint8Array(lengthBytesUTF8(data)+1);var actualNumBytes=stringToUTF8Array(data,buf,0,buf.length);FS.write(stream,buf,0,actualNumBytes,undefined,opts.canOwn)}else if(ArrayBuffer.isView(data)){FS.write(stream,data,0,data.byteLength,undefined,opts.canOwn)}else{throw new Error("Unsupported data type")}FS.close(stream)},cwd:function(){return FS.currentPath},chdir:function(path){var lookup=FS.lookupPath(path,{follow:true});if(lookup.node===null){throw new FS.ErrnoError(2)}if(!FS.isDir(lookup.node.mode)){throw new FS.ErrnoError(20)}var err=FS.nodePermissions(lookup.node,"x");if(err){throw new FS.ErrnoError(err)}FS.cur⟪ 76049 characters skipped ⟫=="undefined"||!(e instanceof FS.ErrnoError))abort(e);return-e.errno}}function ___syscall195(which,varargs){SYSCALLS.varargs=varargs;try{var path=SYSCALLS.getStr(),buf=SYSCALLS.get();return SYSCALLS.doStat(FS.stat,path,buf)}catch(e){if(typeof FS==="undefined"||!(e instanceof FS.ErrnoError))abort(e);return-e.errno}}function ___syscall220(which,varargs){SYSCALLS.varargs=varargs;try{var stream=SYSCALLS.getStreamFromFD(),dirp=SYSCALLS.get(),count=SYSCALLS.get();if(!stream.getdents){stream.getdents=FS.readdir(stream.path)}var struct_size=280;var pos=0;var off=FS.llseek(stream,0,1);var idx=Math.floor(off/struct_size);while(idx<stream.getdents.length&&pos+struct_size<=count){var id;var type;var name=stream.getdents[idx];if(name[0]==="."){id=1;type=4}else{var child=FS.lookupNode(stream.node,name);id=child.id;type=FS.isChrdev(child.mode)?2:FS.isDir(child.mode)?4:FS.isLink(child.mode)?10:8}tempI64=[id>>>0,(tempDouble=id,+Math_abs(tempDouble)>=1?tempDouble>0?(Math_min(+Math_floor(tempDouble/4294967296),4294967295)|0)>>>0:~~+Math_ceil((tempDouble-+(~~tempDouble>>>0))/4294967296)>>>0:0)],HEAP32[dirp+pos>>2]=tempI64[0],HEAP32[dirp+pos+4>>2]=tempI64[1];tempI64=[(idx+1)*struct_size>>>0,(tempDouble

picotron/drive/dumps/ram/mount/einikdkc/debug.lua:
...
picotron/drive/dumps/ram/mount/einikdkc/gui.lua:
...
picotron/drive/dumps/ram/mount/einikdkc/inst.lua:
  521
...
picotron/drive/dumps/ram/mount/einikdkc/main.lua:
  167
  168: --	print(string.format("cpu:%3.3f",stat(1)),440,250,7)
  169  --draw_mudo_state(380,200)

picotron/drive/dumps/ram/mount/einikdkc/track.lua:
  135
...
picotron/drive/dumps/ram/mount/einikdkc/update.lua:
  31
...
picotron/drive/dumps/ram/mount/ejwvdoug/main.lua:
  30  	print("dots:"..max_idx,2,2,13)
  31: 	print("cpu: "..string.format("%.3f",stat(1)),2,12)
  32: 	print(stat(7).."fps",2,24)
  33

picotron/drive/dumps/ram/mount/eormuurg/main.lua:
  123  	local str=string.format("points: %d  cpu: %3.3f (%dfps)",
  124: 		#p, stat(1), stat(7))
  125  	print(str, 11, 11, 0)

picotron/drive/dumps/ram/mount/ffqfjcvi/main.lua:
  123  	local str=string.format("points: %d  cpu: %3.3f (%dfps)",
  124: 		#p, stat(1), stat(7))
  125  	print(str, 11, 11, 0)

picotron/drive/dumps/ram/mount/fillkxcs/main.lua:
  71  	fillp()
  72: 	print(string.format("%.4f %dfps",stat(1), stat(7)),2,2,5)
  73

picotron/drive/dumps/ram/mount/fsgnqwpv/main.lua:
  43  	clip()
...
picotron/drive/dumps/ram/mount/gfrbswob/system/apps/terminal.lua:
  123
  124: 	-- add_line("picotron terminal // "..flr(stat(0)).." bytes used")
  125

picotron/drive/dumps/ram/mount/gfrbswob/system/demos/bells.p64:
  98  	end
  99: 	--print(stat(1),2,2,8)
  100  end

picotron/drive/dumps/ram/mount/gfrbswob/system/demos/carpet.p64:
  146
  147: 	--print(string.format("cpu: %3.3f (%dfps)", stat(1), stat(7)), 10, 10, 1)
  148

picotron/drive/dumps/ram/mount/gfrbswob/system/demos/hexopus.p64:
  54  	clip()
  55: 	print(string.format("%.5f", stat(1)), 12,2,17)
  56  end

picotron/drive/dumps/ram/mount/gfrbswob/system/demos/highway.p64:
  83  	fillp()
  84: 	print(string.format("%.4f %dfps",stat(1), stat(7)),2,2,5)
  85

picotron/drive/dumps/ram/mount/gfrbswob/system/demos/pixeldust.p64:
  48  	print("dots:"..max_idx,2,2,13)
  49: 	print("cpu: "..string.format("%.3f",stat(1)),2,12)
  50: 	print(stat(7).."fps",2,24)
  51

picotron/drive/dumps/ram/mount/gfrbswob/system/demos/proggy.p64:
  253  	-- show fps
  254: 	print(string.format("cpu:%.3f",stat(1)), 420,5,13)
  255  end

picotron/drive/dumps/ram/mount/gfrbswob/system/demos/spiral.p64:
  22  	end
...
picotron/drive/dumps/ram/mount/gfrbswob/system/lib/foot.lua:
  33
...
picotron/drive/dumps/ram/mount/gfrbswob/system/lib/gui_ed.lua:
  1468
...
picotron/drive/dumps/ram/mount/gfrbswob/system/misc/load_player.lua:
  18
...
picotron/drive/dumps/ram/mount/gfrbswob/system/screensavers/breach.p64:
  69
...
picotron/drive/dumps/ram/mount/gtdzmahg/main.lua:
  62  	end
  63: 	--print(stat(1),2,2,8)
  64  end

picotron/drive/dumps/ram/mount/jjkxftqr/main.lua:
  11  	end
  12: 	print(string.format("%.5f",stat(1)),2,2,7)
  13  end

picotron/drive/dumps/ram/mount/lnqujccm/main.lua:
  110
  111: 	local t0=stat(1)
  112
...
picotron/drive/dumps/ram/mount/mefisjvu/main.lua:
  51  	-- show fps
  52: 	print(string.format("cpu:%.3f",stat(1)), 420,5,13)
  53  end

picotron/drive/dumps/ram/mount/mxhduaku/main.lua:
  110
  111: 	local t0=stat(1)
  112
...
picotron/drive/dumps/ram/mount/ntuuewir/main.lua:
  30  	print("dots:"..max_idx,2,2,13)
  31: 	print("cpu: "..string.format("%.3f",stat(1)),2,12)
  32: 	print(stat(7).."fps",2,24)
  33

picotron/drive/dumps/ram/mount/qbqcuchc/main.lua:
  43  	clip()
  44: 	print(string.format("%.5f", stat(1)), 12,2,17)
  45  end

picotron/drive/dumps/ram/mount/qljraddm/main.lua:
  11  function _draw()
  12: 	--printh("_draw() start "..stat(1))
  13  	cls()
...
picotron/drive/dumps/ram/mount/rlnzdsxi/debug.lua:
  10  		-- chan->playing_track_index
  11: 		print("\14"..stat(400+i,12), sx, sy, 7)
  12
  13  		-- chan->inst
...
picotron/drive/dumps/ram/mount/rlnzdsxi/gui.lua:
  328  		draw=function(self)
  329: 			local playing_pattern = stat(466)
  330  			i0 = mid(0, ((-contents.y) \ 12) * 4, 127)
...
picotron/drive/dumps/ram/mount/rlnzdsxi/inst.lua:
  521
  522: 			local tick_len = stat(400 + ci_channel, 8)
  523
...
picotron/drive/dumps/ram/mount/rlnzdsxi/main.lua:
  167
  168: --	print(string.format("cpu:%3.3f",stat(1)),440,250,7)
  169  --draw_mudo_state(380,200)

picotron/drive/dumps/ram/mount/rlnzdsxi/track.lua:
  135
  136: 	if (stat(464)==0) return nil -- nothing playing
  137
  138  	for i=0,15 do
...
picotron/drive/dumps/ram/mount/rlnzdsxi/update.lua:
  31
  32: 		local row = stat(400 + stat(467), 9)
  33  		pattern_contents.y = mid(0, 64 - row * 8,
...
picotron/drive/dumps/ram/mount/rtugaoqh/canvas.lua:
  45  	--[[
  46: 	if (stat(1) - fill_cpu > 2) then
  47: 		fill_cpu = stat(1) gui:draw_all() flip()
  48  		poke(0x547c, 1) -- keep holding frame

picotron/drive/dumps/ram/mount/rtugaoqh/gui.lua:
  247  	function el:draw()
  248: 		print(string.format("%2.2fmb (%02d%%)",stat(0)/0x100000,stat(0)\167772),0,0,
  249: 			stat(0) > 0xe00000 and 14 or 21) -- warn when more than 7/8ths of capacity
  250  	end

picotron/drive/dumps/ram/mount/rtugaoqh/undo.lua:
  38  	local ii = item[current_item]
  39: 	local tt0 = stat(1)
  40  	ii.undo_stack:checkpoint()
...
picotron/drive/dumps/ram/mount/ruvopuku/main.lua:
  42  	end
  43: 	print(string.format("cpu:%.3f",stat(1)),2,2)
  44  end

picotron/drive/dumps/ram/mount/rwpgwskn/main.lua:
  62  	end
  63: 	--print(stat(1),2,2,8)
  64  end

picotron/drive/dumps/ram/mount/tduwqjry/main.lua:
  35
  36: 	--print(stat(1),400,2,7)
  37  	--circ(mx,my,rr/2,7)

picotron/drive/dumps/ram/mount/tjgvywrj/main.lua:
  71  	fillp()
  72: 	print(string.format("%.4f %dfps",stat(1), stat(7)),2,2,5)
  73

picotron/drive/dumps/ram/mount/tukxcwqu/canvas.lua:
   43  	--[[
...
picotron/drive/dumps/ram/mount/tukxcwqu/gui.lua:
...
picotron/drive/dumps/ram/mount/tukxcwqu/undo.lua:
...
picotron/drive/dumps/ram/mount/txsddmza/main.lua:
  134
  135: 	--print(string.format("cpu: %3.3f (%dfps)", stat(1), stat(7)), 10, 10, 1)
  136

picotron/drive/dumps/ram/mount/vvgjapgd/main.lua:
  51  	-- show fps
  52: 	print(string.format("cpu:%.3f",stat(1)), 420,5,13)
  53  end

picotron/drive/dumps/ram/mount/zfdzyukc/canvas.lua:
   43  	--[[
   44: 	if (stat(1) - fill_cpu > 2) then
   45: 		fill_cpu = stat(1) gui:draw_all() flip()
   46  		poke(0x547c, 1) -- keep holding frame
...
picotron/drive/dumps/ram/mount/zfdzyukc/gui.lua:
  167  	function el:draw()
  168: 		print(string.format("%2.2fmb (%02d%%)",stat(0)/0x100000,stat(0)\167772),0,0,
  169: 			stat(0) > 0xe00000 and 14 or 21) -- warn when more than 7/8ths of capacity
  170  	end

picotron/drive/dumps/ram/mount/zfdzyukc/undo.lua:
  34  	local ii = item[current_item]
  35: 	local tt0 = stat(1)
  36
...
picotron/drive/dumps/ram/mount/zsovbgnc/main.lua:
  134
  135: 	--print(string.format("cpu: %3.3f (%dfps)", stat(1), stat(7)), 10, 10, 1)
  136

picotron/drive/dumps/ram/mount/zvoojjub/main.lua:
  42  	end
  43: 	print(string.format("cpu:%.3f",stat(1)),2,2)
  44  end
...
picotron/drive/dumps/system/apps/gfx.p64:
    49  	--[[
...
picotron/drive/dumps/system/apps/map.p64:
    47  	--[[
...
picotron/drive/dumps/system/apps/sfx.p64:
...
picotron/drive/dumps/system/apps/terminal.lua:
  123
  124: 	-- add_line("picotron terminal // "..flr(stat(0)).." bytes used")
  125

picotron/drive/dumps/system/demos/bells.p64:
  98  	end
  99: 	--print(stat(1),2,2,8)
  100  end

picotron/drive/dumps/system/demos/carpet.p64:
  146
  147: 	--print(string.format("cpu: %3.3f (%dfps)", stat(1), stat(7)), 10, 10, 1)
  148

picotron/drive/dumps/system/demos/hexopus.p64:
  54  	clip()
  55: 	print(string.format("%.5f", stat(1)), 12,2,17)
  56  end

picotron/drive/dumps/system/demos/highway.p64:
  83  	fillp()
  84: 	print(string.format("%.4f %dfps",stat(1), stat(7)),2,2,5)
  85

picotron/drive/dumps/system/demos/pixeldust.p64:
  48  	print("dots:"..max_idx,2,2,13)
  49: 	print("cpu: "..string.format("%.3f",stat(1)),2,12)
  50: 	print(stat(7).."fps",2,24)
  51

picotron/drive/dumps/system/demos/proggy.p64:
  253  	-- show fps
  254: 	print(string.format("cpu:%.3f",stat(1)), 420,5,13)
  255  end

picotron/drive/dumps/system/demos/spiral.p64:
  22  	end
  23: 	print(string.format("%.5f",stat(1)),2,2,7)
  24  end
...
picotron/drive/dumps/system/lib/foot.lua:
  33
  34: -- printh("entering mainloop at cpu:"..flr(stat(1)/100).."%   mem:"..flr(stat(0)/1024).."k")
  35
...
picotron/drive/dumps/system/lib/gui_ed.lua:
  1468
  1469: --		printh(string.format("cpu %.3f",stat(1)),440,2,7)
  1470
...
picotron/drive/dumps/system/screensavers/breach.p64:
  69
  70: 	--print(stat(1),400,2,7)
  71  	--circ(mx,my,rr/2,7)

picotron/drive/dumps/system/screensavers/bunny.p64:
  178  function _draw()
...
picotron/drive/dumps/system/screensavers/dizzy.p64:
  135  	local str=string.format("points: %d  cpu: %3.3f (%dfps)",
  136: 		#p, stat(1), stat(7))
  137  	print(str, 11, 11, 0)

picotron/drive/dumps/system/screensavers/swoosh.p64:
  53  	end
  54: 	print(string.format("cpu:%.3f",stat(1)),2,2)
  55  end

picotron/drive/dumps/system/util/load.lua:
   48  	-- convert to UTC
...
picotron/drive/dumps/system/util/save.lua:
  51
